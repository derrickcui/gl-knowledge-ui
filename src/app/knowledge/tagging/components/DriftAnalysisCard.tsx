  ...
  return (
    <section className="rounded-2xl border border-slate-700 bg-slate-900/80 p-5">
      ...
      <p className="mt-1 text-xs text-slate-400">
        Compare changed doc-topic cells between runtime versions.
      </p>
      {!drift ? (
        <div className="mt-4 text-xs text-slate-400">No drift data.</div>
      ) : (
        <>
            ...
            <table className="min-w-full text-xs">
              <thead>
                <tr className="text-slate-400">
                  <th className="px-2 py-2 text-left">docId</th>
                  <th className="px-2 py-2 text-left">topicId</th>
                  <th className="px-2 py-2 text-left">status</th>
                  <th className="px-2 py-2 text-left">fromWeight</th>
                  <th className="px-2 py-2 text-left">toWeight</th>
                </tr>
              </thead>
              <tbody>
                {rows.map((item, idx) => (
          ...
          {rows[0] ? (
            ...
            >
              Open changed docs in Governance
            </Link>
          ) : null}
        </>
      )}
